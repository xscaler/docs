...
The step sets how many data points come back and how much compute the query costs.
...
| For rate functions, match the range to at least 4× the step | `rate(metric[5m])` works well with a step of `60s` to `90s`. |
