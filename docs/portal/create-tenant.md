...
5. The portal shows the tenant credentials. **Copy them now**:
   ...
   The portal shows the API token once, at creation. If you lose it, [rotate the key](/portal/api-tokens#rotate-an-api-token) for a new one.
   :::
...
You find the tenant ID in the tenants list and on the tenant detail page.
