---
id: legal
title: Legal & Open Source Notices
displayed_sidebar: mainSidebar
---

# Legal & Open Source Notices

## 1. Overview

This platform provides a managed observability service for monitoring and analyzing system performance and metrics.

For full details on how we handle your data and your rights, please refer to our dedicated policy pages:

- [Privacy Policy](https://xscalerlabs.com/privacy)
- [Terms & Conditions](https://xscalerlabs.com/terms)
- [Security](https://xscalerlabs.com/security)
- [Cookie Policy](https://xscalerlabs.com/cookies)

---

## 2. Open Source Software

This product includes open-source software components, including:

* Grafana Mimir
* Grafana
* Prometheus

These components are used in accordance with their respective licenses, including the AGPLv3 and Apache License 2.0.

---

## 3. No Affiliation

Grafana, Prometheus, and related marks are trademarks of their respective owners.

This product is not affiliated with, endorsed by, or sponsored by these organizations.

---

## 4. Disclaimer of Warranties

This service includes third-party open-source software components that are provided "as is", without warranties of any kind.

---

## 5. Limitation of Liability

To the maximum extent permitted by law, we are not liable for any indirect or consequential damages arising from the use of this platform.

---

## 6. Updates

We may update this page from time to time.

---

## 7. Contact

For legal inquiries, contact:
**[info@xscalerlabs.com](mailto:info@xscalerlabs.com)**
