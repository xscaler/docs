...
Collect mongod log including queries, connections, and replica set events. Add the following to your Alloy config:

```river
local.file_match "mongodb_logs" {
  path_targets = [{
...
loki.source.file "mongodb_logs" {
  targets    = local.file_match.mongodb_logs.targets
